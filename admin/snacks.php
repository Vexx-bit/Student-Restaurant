...
if (isset($_POST['insertMeal'])) {
    ...
    if (mysqli_query($conn, $insertMealQuery)) {
        echo "<script>alert('New snack added successfully!')</script>";
...
if (isset($_GET['deleteMeal'])) {
    ...
    if (mysqli_query($conn, $deleteMealQuery)) {
        echo "<script>alert('Snack deleted successfully!');</script>";
        echo "<script>window.location.href='snacks.php';</script>";
        exit();
    } else {
...
<body>
    ...
    <div class="container main-container mt-4 table-responsive">
        <h1>Snacks</h1>
        <button class="add-meal-btn btn btn-primary mb-3" data-bs-toggle="modal" data-bs-target="#addMealModal">Add a New Snack <i class="bx bx-plus"></i></button>
        ...
        <table class="table table-striped">
            ...
            <tbody>
                ...
                $fetchMealsQuery = "SELECT * FROM menu WHERE meal_category = 'snack'";
                ...
                while ($row = mysqli_fetch_assoc($mealsResult)) {
                    echo '<tr>
                            ...
                            <td>
                                <a href="edit_snack.php?id=' . $row['meal_id'] . '" class="bx bx-edit action-icons"></a>
                                <a href="snacks.php?deleteMeal=' . $row['meal_id'] . '" class="bx bx-trash action-icons text-danger" onclick="return confirm(\'Are you sure you want to delete this snack?\')"></a>
                            </td>
                        </tr>';
                }
                ?>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header" style="background: var(--color-navy-blue);color: var(--color-text-white);">
                    <h5 class="modal-title" id="addMealModalLabel">Add a New Snack</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form method="POST" enctype="multipart/form-data">
                        <input type="hidden" name="mealCategory" value="snack"> <!-- Hidden input for meal category -->
                        <div class="mb-3">
                            <label for="mealTitle" class="form-label">Snack Title</label>
                            <input type="text" class="form-control" id="mealTitle" name="mealTitle" required>
                        </div>
                        <div class="mb-3">
                            <label for="mealPrice" class="form-label">Snack Price(KES)</label>
                            <input type="number" class="form-control" id="mealPrice" name="mealPrice" step="0.01" required>
                        </div>
                        <div class="mb-3">
                            <label for="mealImage" class="form-label">Snack Image</label>
                            <input type="file" class="form-control" id="mealImage" name="mealImage" accept="image/*" required>
                        </div>
                        <button type="submit" class="btn btn-primary" name="insertMeal">Add Snack</button>
                    </form>
                </div>
            </div>
        </div>
