...
if (isset($_GET['id'])) {
    ...
    $fetchMealQuery = "SELECT * FROM menu WHERE meal_id = $mealID AND meal_category = 'main'";
    $mealResult = mysqli_query($conn, $fetchMealQuery);
    $mealData = mysqli_fetch_assoc($mealResult);

    if (!$mealData) {
        echo "<script>alert('Meal not found!'); window.location.href='main_dishes.php';</script>";
        exit();
    }
}
...
if (isset($_POST['updateMeal'])) {
    ...
    $mealImage = $mealData['meal_img']; // Keep existing image
    ...
    $updateMealQuery = "UPDATE menu SET meal_title='$mealTitle', meal_price='$mealPrice', meal_img='$mealImage' WHERE meal_id=$mealID AND meal_category='main'";
    if (mysqli_query($conn, $updateMealQuery)) {
        echo "<script>alert('Meal updated successfully!'); window.location.href='main_dishes.php';</script>";
        exit();
    } else {
    ...
    <div class="container mt-5 mb-5">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="main_dishes.php">Main Dishes</a></li>
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <button type="submit" class="btn btn-primary" name="updateMeal">Update Meal</button>
        </form>
    </div>
