...
if (isset($_POST['insertMeal'])) {
    ...
    if (mysqli_query($conn, $insertMealQuery)) {
        echo "<script>alert('New grocery added successfully!')</script>";
        echo "<script>window.location.href='groceries.php';</script>";
        exit();
    } else {
...
if (isset($_GET['deleteMeal'])) {
    ...
    if (mysqli_query($conn, $deleteMealQuery)) {
        echo "<script>alert('Grocery deleted successfully!');</script>";
        echo "<script>window.location.href='groceries.php';</script>";
        exit();
    } else {
...
<body>
    ...
    <div class="container main-container mt-4 table-responsive">
        <h1>Groceries</h1>
        <button class="add-meal-btn btn btn-primary mb-3" data-bs-toggle="modal" data-bs-target="#addMealModal">Add a New Grocery <i class="bx bx-plus"></i></button>
        ...
        <table class="table table-striped">
            ...
            <tbody>
                ...
                $fetchMealsQuery = "SELECT * FROM menu WHERE meal_category = 'grocery'";
                ...
                while ($row = mysqli_fetch_assoc($mealsResult)) {
                    echo '<tr>
                            ...
                            <td>
                                <a href="edit_groceries.php?id=' . $row['meal_id'] . '" class="bx bx-edit action-icons"></a>
                                <a href="groceries.php?deleteMeal=' . $row['meal_id'] . '" class="bx bx-trash action-icons text-danger" onclick="return confirm(\'Are you sure you want to delete this grocery?\')"></a>
                            </td>
                        </tr>';
                }
                ?>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header" style="background: var(--color-navy-blue);color: var(--color-text-white);">
                    <h5 class="modal-title" id="addMealModalLabel">Add a New Grocery</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form method="POST" enctype="multipart/form-data">
                        <input type="hidden" name="mealCategory" value="grocery"> <!-- Hidden input for meal category -->
                        <div class="mb-3">
                        ...
                        <button type="submit" class="btn btn-primary" name="insertMeal">Add Grocery</button>
                    </form>
