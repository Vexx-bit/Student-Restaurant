...
if (isset($_GET['id'])) {
    ...
    $fetchMealQuery = "SELECT * FROM menu WHERE meal_id = $mealID AND meal_category = 'beverage'";
    $mealResult = mysqli_query($conn, $fetchMealQuery);
    $mealData = mysqli_fetch_assoc($mealResult);

    if (!$mealData) {
        echo "<script>alert('Beverage not found!'); window.location.href='beverages.php';</script>";
        exit();
    }
}
...
if (isset($_POST['updateMeal'])) {
    ...
    $updateMealQuery = "UPDATE menu SET meal_title='$beverageTitle', meal_price='$beveragePrice', meal_img='$beverageImage' WHERE meal_id=$mealID AND meal_category='beverage'";
    if (mysqli_query($conn, $updateMealQuery)) {
        echo "<script>alert('Beverage updated successfully!'); window.location.href='beverages.php';</script>";
        exit();
    } else {
    ...
    <div class="container mt-5 mb-5">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="beverages.php">Beverages</a></li>
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <button type="submit" class="btn btn-primary" name="updateMeal">Update Beverage</button>
        </form>
    </div>
